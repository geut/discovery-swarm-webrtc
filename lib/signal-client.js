...
class Request {
  ...
  get initiator () {
    return Buffer.from(this._simpleRequest.initiator)
  }

  get channel () {
    return Buffer.from(this._simpleRequest.metadata.channel)
  }

  get connectionId () {
    return Buffer.from(this._simpleRequest.metadata.connectionId)
  }
...
class SignalClient extends EventEmitter {
  ...
  async candidates (id, channel) {
    return this._emitTransaction('simple-signal[candidates]', { id, channel }, ({ peers = [] }) => {
      return peers.map(id => Buffer.from(id))
    })
  }

  async leave (id, channel) {
    return this._emitTransaction('simple-signal[leave]', { id, channel })
  }
  ...
  discover (id, channel) {
    return this._simpleSignal.discover({ id, channel })
  }

  async connect (peer, simplePeer) {
    try {
      const { peer: socket } = await this._simpleSignal.connect(peer.id, { channel: peer.channel, connectionId: peer.connectionId }, simplePeer)
      return socket
    } catch (err) {
      throw SignalClient.parseMetadataError(err)
    }
  ...
  _initialize () {
    ...
    this._simpleSignal.on('discover', ({ channel, peers = [] }) => this.emit('discover', {
      ...
      peers: peers.map(id => Buffer.from(id))
    }))
